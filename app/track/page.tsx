      ...
      {steps.map((step, idx) => {
        ...
        return (
          <div key={step.key} style={{ display:'flex', alignItems:'flex-start', flex:'1 0 auto', minWidth:46 }}>
            ...
            {idx < steps.length-1 && (
              <div style={{ height:2, flex:'0 0 10px', marginTop:15, background: isPast?'#10b981':'#e5e7eb' }} />
            )}
          </div>
        )
      })}
...
export default function TrackPage() {
  ...
  return (
    <div style={{ maxWidth:600, margin:'0 auto', padding:'40px 20px 80px' }}>
      ...
      <p style={{ color:'#6b7280', fontSize:14, marginBottom:20 }}>접수 안내 메일로 받으신 <b>고객 확인 번호</b>를 입력하시면 현재 진행 단계를 확인하실 수 있습니다.</p>
      ...
      {err && (
        <div style={{ marginTop:16, padding:'12px 16px', background:'#fef2f2', border:'1px solid #fca5a5', borderRadius:10, color:'#b91c1c', fontSize:14 }}>
          {err}
        </div>
      )}

      {data && (
        <div style={{ marginTop:24, border:'1px solid #e5e7eb', borderRadius:14, overflow:'hidden' }}>
          ...
          <div style={{ padding:'18px' }}>
            {data.status === 'rejected' ? (
              <div style={{ padding:'14px 16px', background:'#fef2f2', border:'1px solid #fecaca', borderRadius:10, color:'#b91c1c', fontSize:14 }}>
                이 견적은 진행이 어려워 거절 처리되었습니다. 자세한 내용은 담당자에게 문의해 주세요.
              </div>
            ) : (
              <>
                <Milestone data={data} />
                <div style={{ marginTop:14, fontSize:13, color:'#6b7280', lineHeight:1.8 }}>
      ...
      {searched && !data && !err && !loading && (
        <div style={{ marginTop:16, color:'#9ca3af', fontSize:14 }}>조회 결과가 없습니다.</div>
      )}
    </div>
  )
}
