...
function parseBinarySTL(buffer: ArrayBuffer): Float32Array {
  ...
  for (let i = 0; i < n; i++) {
    offset += 12
    for (let j = 0; j < 9; j++) { verts[i*9+j] = view.getFloat32(offset, true); offset += 4 }
    offset += 2
  }
  return verts
}
...
function parseSTL(buffer: ArrayBuffer): Float32Array {
  ...
  if (header === 'solid' && !isBinarySTL(buffer)) return parseASCIISTL(new TextDecoder().decode(buffer))
  return parseBinarySTL(buffer)
}
function calcSTLVolume(v: Float32Array): number {
  let vol = 0
...
function calcBBox(v: Float32Array) {
  ...
  return { x:parseFloat((mxX-mnX).toFixed(1)), y:parseFloat((mxY-mnY).toFixed(1)), z:parseFloat((mxZ-mnZ).toFixed(1)) }
}
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const volume = calcSTLVolume(verts)
        const stlInfo = { ...bbox, volume }
        setInfo(stlInfo); onAnalyzed(stlInfo)

        const w = mountRef.current!.clientWidth, h = 280
        ...
        const d2 = new THREE.DirectionalLight(0x8ab4f8, 0.4); d2.position.set(-2,-1,-1); scene.add(d2)

        const geo = new THREE.BufferGeometry()
        geo.setAttribute('position', new THREE.BufferAttribute(verts, 3))
        geo.computeVertexNormals()
        const mat = new THREE.MeshPhongMaterial({ color:0x2563eb, specular:0x4488ff, shininess:40, side:THREE.DoubleSide })
        ...
        camera.position.set(maxDim*1.5, maxDim, maxDim*1.5); camera.lookAt(0,0,0)
        const grid = new THREE.GridHelper(maxDim*3, 10, 0xd1d5db, 0xe5e7eb)
        grid.position.y = -size.z/2; scene.add(grid)
    ...
    run()
    return () => { cancelAnimationFrame(animId); renderer?.dispose(); if(mountRef.current) mountRef.current.innerHTML='' }
  }, [file])
  ...
  return (
    <div style={{ borderRadius:12, overflow:'hidden', border:'1.5px solid #e5e7eb', marginBottom:16 }}>
      <div style={{ position:'relative', background:'#f9fafb', height:280 }}>
        {loading && <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:10, color:'#6b7280' }}><div style={{fontSize:32}}>⏳</div><div style={{fontSize:13}}>3D 모델 분석 중...</div></div>}
        {err && <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:8, color:'#9ca3af' }}><div style={{fontSize:32}}>📄</div><div style={{fontSize:13}}>미리보기를 불러올 수 없습니다</div></div>}
        <div ref={mountRef} style={{ width:'100%', height:'100%' }} />
        {!loading && !err && <div style={{ position:'absolute', bottom:8, right:10, fontSize:11, color:'#9ca3af', background:'rgba(255,255,255,0.8)', padding:'3px 8px', borderRadius:6 }}>🖱 드래그: 회전 · 스크롤: 확대</div>}
      </div>
      {info && (
      ...
      <div style={S.card}>
        ...
        <div style={S.body}>
          ...
          {step===1&&<>
            ...
            <div onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)}
              ...
              style={{border:`2px dashed ${drag?'#2563eb':form.file?'#16a34a':'#d1d5db'}`,borderRadius:12,
                ...
                marginBottom:showViewer?12:16,background:drag?'#eff6ff':form.file?'#f0fdf4':'#f9fafb',transition:'all .15s'}}>
            ...
            {form.file&&!showViewer&&(
              <div style={{...S.alert,background:'#fffbeb',border:'1px solid #fcd34d',color:'#92400e',marginBottom:16}}>
                <span>ℹ️</span><span>STL 파일은 3D 미리보기와 자동 사이즈/부피 계산이 지원됩니다. 현재 파일은 담당자가 직접 분석 후 견적을 산출합니다.</span>
              </div>
            )}
            <div style={S.grp}>
          ...
          {step===2&&<>
            ...
            {disc>0&&<div style={{...S.alert,background:'#f0fdf4',border:'1px solid #86efac',color:'#14532d'}}>✅ <span>{form.qty}개 이상 주문 — <b>{disc}% 수량 할인</b> 자동 적용됩니다</span></div>}
          ...
          {step===3&&<>
            ...
            <div style={{background:'#f9fafb',borderRadius:12,padding:'14px 18px',marginBottom:14}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
                ...
                {form.sizeX!=null&&<><div><div style={{fontSize:11,color:'#9ca3af',marginBottom:2}}>X (가로)</div><div style={{fontSize:13,fontWeight:600}}>{form.sizeX} mm</div></div>
            ...
            <div style={{...S.alert,background:'#fffbeb',border:'1px solid #fcd34d',color:'#92400e'}}>
              <span>⚠️</span><span>위 금액은 파일 부피 기반 <b>자동 계산 예상 견적</b>입니다. 담당자 검토 후 <b>확정 견적을 이메일로 안내</b>드립니다.</span>
            </div>
